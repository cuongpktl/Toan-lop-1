...
const DecodeMath: React.FC<Props> = ({ problem, onUpdate, showResult }) => {
  ...
  return (
    <div className={`p-6 sm:p-10 rounded-[48px] border-4 bg-white shadow-xl transition-all flex flex-col items-center gap-8 ${
      ...
      <div className="w-full overflow-x-auto no-scrollbar py-4 scroll-smooth">
        <div className="flex items-center justify-center gap-4 sm:gap-12 min-w-max mx-auto px-4">
      ...
      {isCorrect ? (
        <div className="text-green-600 font-black animate-fadeIn text-lg flex items-center gap-2 text-center">
          <span>🌟</span> Bé giải mã và tính toán xuất sắc!
        </div>
      ) : (
        <p className="text-gray-400 font-bold text-sm italic text-center">
